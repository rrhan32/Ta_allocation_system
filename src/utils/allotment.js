...
// Validation function to check and adjust assignments if necessary
  function validateAssignments() {
    for (const professor of professors) {
      for (const pref of professor.preferences) {
        const taRollNumber = pref.taRollNumber;
        const courseCode = professor.courseCode;
        const weight = pref.weight;

        if (!taAssigned[taRollNumber]) {
          // Find available TAs for the professor's course
          const availableTAs = tas.filter(
            ta =>
              ta.preferences.some(p => p.courseCode === courseCode) &&
              !specificallyChosenByOtherProf(ta.rollNumber, courseCode)
          );

          if (availableTAs.length > 0) {
            // Sort available TAs by preference weight
            availableTAs.sort((a, b) => {
              const weightA = a.preferences.find(p => p.courseCode === courseCode).weight;
              const weightB = b.preferences.find(p => p.courseCode === courseCode).weight;
              return weightB - weightA;
            });

            // Assign the TA with the highest preference to the professor
            const highestPreferenceTA = availableTAs[0];
            matching.find(m => m.courseCode === courseCode).expertTa.push(highestPreferenceTA.rollNumber);
            taAssigned[highestPreferenceTA.rollNumber] = true;
          }
        }
      }
    }

    // Additional validation: Check if a professor with higher preference for a specific course is not assigned a qualified TA
    for (const professor of professors) {
      for (const pref of professor.preferences) {
        const taRollNumber = pref.taRollNumber;
        const courseCode = pref.courseCode;

        if (!taAssigned[taRollNumber]) {
          const qualifiedTAs = tas.filter(
            ta =>
              ta.preferences.some(p => p.courseCode === courseCode) &&
              !specificallyChosenByOtherProf(ta.rollNumber, courseCode) 
          ...
          if (qualifiedTAs.length > 0) {
            // Assign the qualified TA to the professor
            const qualifiedTA = qualifiedTAs[0];
            matching.find(m => m.courseCode === courseCode).expertTa.push(qualifiedTA.rollNumber);
            taAssigned[qualifiedTA.rollNumber] = true;
          }
        }
      }
    }
  ...
  validateAssignments(); // Call the validation function before returning the assignments
